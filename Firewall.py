...
class Firewall:
    def __init__(self, csv_file):
        ...
        port_modified_low, port_modified_high = self.get_number_range(port_raw)
        ip_address_raw = self.csv["ip_address"]
        ip_address_modified_low, ip_address_modified_high = self.get_number_range(ip_address_raw)
    ...
    def accept_packet(self, direction, protocol, port, ip_address):
        ...
        ip_low, ip_high = self.get_number_range([ip_address])
    ...
    def get_number_range(self, column):
        ...
        for string in column:
            ...
            dash = string.find("-")
            if dash == -1:
                low[index] = self.get_number(string)
                high[index] = low[index]
            else:
                low[index] = self.get_number(string[:dash])
                high[index ]= self.get_number(string[dash + 1:])
            index += 1
        return low, high
